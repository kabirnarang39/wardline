...
func (alwaysDenyLimiter) Allow(identity string, now time.Time) domain.Verdict {
...
func TestChecker_FlagOffAlwaysAllows(t *testing.T) {
	...
	got := c.Check("agent-abc123", time.Now())
	if !got.Allowed {
...
func (r *recordingLimiter) Allow(identity string, now time.Time) domain.Verdict {
