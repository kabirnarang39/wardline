...
| Prometheus metrics: baseline allow path with recording on | 300 req/s | 100% | 0.29ms | 0.72ms | 1.20ms |
| Prometheus metrics, race-instrumented build, postgres+budget+anomaly combo | 30 req/s × 3 identities, ~4 min (20,770 requests) | 100%, 0 races, exact bookkeeping (metrics counter matched audit count and expected total once settled) | — | — | — |
| Prometheus metrics, 2 replicas sharing one Postgres pool | 15 + 5 req to replica A/B respectively | Each replica's own `/metrics` reflects only its own local traffic (15 vs. 5) — correct per-instance isolation, not a shared/aggregated counter | — | — | — |
| Log retention: real backdated entry purged, fresh entries kept | 1 backdated (10-day-old) entry + 2 fresh entries | 100% correct: backdated entry purged on the first tick, both fresh entries survived | — | — | — |
| Compliance scheduled export: real proxied calls captured per window | 1-8 requests per 3s export window | 100% correct — every request is captured by the export window it falls in, including one landing in the same wall-clock second as a window boundary | — | — | — |
| `infer-policy`: generated policy round-tripped into a fresh instance | 2 allow + 1 deny real audit entries | 100% correct: 2 rules generated (denied entry excluded), fresh instance loading the generated file allowed both observed calls, denied an unobserved tool and the unobserved identity | — | — | — |
| `policy-pack`: list/show/install/overwrite-refusal, installed pack loaded into a fresh instance | 1 pack template | 100% correct: refuses to overwrite an existing output file; an unreplaced placeholder identity matches literally (as documented) while any other identity correctly falls through to `default: deny` | — | — | — |
