...
| `tenant_anomaly.enabled` / `.rate_multiplier` / `.min_calls` | bool/float/int | Detects a coordinated call-volume spike aggregated across every identity in a tenant — closes the gap no per-identity heuristic can (see "Adversarial scenarios"). Logs only, never auto-blocks: there is no single identity to block for a tenant-level signal. In-memory only, no Postgres persistence yet. |
