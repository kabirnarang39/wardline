...
// not from the body.
func ParseToolCall(identity string, body []byte) (domainToolCall, error) {
	var env jsonRPCEnvelope
	if err := json.Unmarshal(body, &env); err != nil {
		return domainToolCall{}, fmt.Errorf("parse json-rpc body: %w", err)
	}
	if env.Method != "tools/call" {
		return domainToolCall{}, fmt.Errorf("unsupported method %q", env.Method)
	}
	return domainToolCall{Identity: identity, Tool: env.Params.Name}, nil
}
```

Note: `domainToolCall` above is a placeholder name to force the correct import — replace it with the actual import in the real file:
...
Use the second (import-correct) version as the actual file content.
