...
// failure -> 403; next is never called on either failure path. Both
// failure paths log a warning (remote-addr only on 401, plus the
// resolved identity on 403 since it's safe/useful once known) — see
// proxy/adapter.Handler's identical "identity authentication failed"
// logging, which this mirrors one layer up.
func RequirePermission(checker Checker, identity IdentityResolver, tenant string, perm domain.Permission, next http.Handler, logger *slog.Logger) http.Handler {
